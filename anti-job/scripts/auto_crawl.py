...
def main():
    print("="*70)
    print("🚀 自动爬取蚂蚁国际招聘所有岗位")
    print("="*70)
    
    project_root = Path(__file__).parent.parent
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = project_root / "output" / f"auto_crawl_{timestamp}"
    ...
    stats = {
        ...
        "category_counts": {name: 0 for name in category_mapping.values()}
    }
    ...
    try:
        ...
        stats["total_positions"] = data.get("totalCount", 0)
        ...
        # 计算总页数
        stats["total_pages"] = (stats["total_positions"] + page_size - 1) // page_size
        print(f"📊 总页数: {stats['total_pages']}")
        print(f"⏰ 预计耗时: {stats['total_pages'] * 2}秒 (每页2秒)")
        
        # 保存第一页
        save_page_data(data, 1, output_dir)
        page_positions = save_positions(content, 1, output_dir, category_mapping, stats)
        stats["positions_saved"] += page_positions
        
        # 爬取剩余页面
        print(f"\n🚀 开始爬取剩余页面...")
        
        for page_no in range(2, stats["total_pages"] + 1):
            print(f"\n📄 处理第 {page_no}/{stats['total_pages']} 页")
            
            # 添加延迟
            time.sleep(2)  # 2秒延迟，避免请求过快
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success") is True:
                        ...
                        # 保存岗位数据
                        page_positions = save_positions(content, page_no, output_dir, category_mapping, stats)
                        stats["positions_saved"] += page_positions
                        
                        print(f"✅ 第{page_no}页: {len(content)}个岗位，累计{stats['positions_saved']}/{stats['total_positions']}")
                    else:
            ...
            # 显示进度
            progress = (page_no / stats["total_pages"]) * 100
            print(f"📈 进度: {page_no}/{stats['total_pages']}页 ({progress:.1f}%)")
        
        # 生成报告
        generate_report(output_dir, stats, timestamp, category_mapping)
...
def save_positions(positions, page_no, output_dir, category_mapping, stats):
    """保存岗位数据"""
    ...
    for position in positions:
        try:
            ...
            for cat in categories:
                cat_str = str(cat)
                mapped_name = category_mapping.get(cat_str, cat_str)
                category_names.append(mapped_name)
                
                # 统计类别数量
                if mapped_name in stats["category_counts"]:
                    stats["category_counts"][mapped_name] += 1
...
def generate_report(output_dir, stats, timestamp, category_mapping):
    """生成爬取报告"""
    ...
    report = {
        ...
        "stats": {
            ...
            "success_rate": ((stats["total_pages"] - stats["failed_pages"]) / stats["total_pages"] * 100) if stats["total_pages"] > 0 else 0,
            "output_directory": str(output_dir),
            "start_time": stats["start_time"].isoformat(),
            "end_time": end_time.isoformat()
        },
        "category_summary": {
            "main_categories": len(category_mapping),
            "sub_categories": 108,
    ...
    print(f"📄 总页数: {stats['total_pages']}")
    print(f"💾 保存岗位: {stats['positions_saved']}/{stats['total_positions']}")
    print(f"❌ 失败页面: {stats['failed_pages']}")
    
    print(f"\n🎯 类别分布:")
    for category, count in stats["category_counts"].items():
        if count > 0:
            percentage = (count / stats["positions_saved"] * 100) if stats["positions_saved"] > 0 else 0
