...
def make_request(api_url, headers, cookies, request_body, page_no):
    """发送HTTP请求，处理编码问题"""
    try:
        # 确保headers中的字符串都是ASCII或UTF-8编码
        safe_headers = {}
        ...
        for key, value in cookies.items():
            safe_key = str(key)
            safe_value = str(value)
            safe_cookies[safe_key] = safe_value
        ...
        for key, value in headers.items():
            try:
                ascii_key = key.encode('ascii', 'ignore').decode('ascii')
                ascii_value = value.encode('ascii', 'ignore').decode('ascii')
                ascii_headers[ascii_key] = ascii_value
            except:
                # 如果无法转换，使用原始值
...
def main():
    ...
    output_dir = project_root / "output" / f"robust_crawl_{timestamp}"
    ...
    try:
        response = make_request(api_url, headers, cookies, request_body, 1)
        ...
        for page_no in range(2, total_pages + 1):
            ...
            try:
                response = make_request(api_url, headers, cookies, request_body, page_no)
...
def save_page_data(data, page_no, output_dir):
    ...
    try:
        ...
        print(f"💾 第{page_no}页数据已保存: {filename}")
        return filepath
    except Exception as e:
...
def generate_report(output_dir, total_count, positions_saved, total_pages, failed_pages, timestamp):
    ...
    if total_pages > 0:
        success_rate = ((total_pages - failed_pages) / total_pages * 100)
        print(f"✅ 成功率: {success_rate:.1f}%")
    print(f"📁 输出目录: {output_dir}")
    print("="*70)

if __name__ == "__main__":
    print("⚠️  注意: 这将爬取所有岗位（约39页），可能需要几分钟时间")
