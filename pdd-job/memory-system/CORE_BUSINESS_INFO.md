...
**项目名称**: pdd-job  
**创建日期**: 2026-05-22  
**最后更新**: 2026-05-22 15:27  
**项目状态**: 活跃  
**项目版本**: v1.0.0  
**项目类型**: 数据爬取  
**项目负责人**: 用户  
**目标公司**: 拼多多集团  
**数据源**: 拼多多社招网站 (https://careers.pddglobalhr.com)

## 🎯 项目目标
### 核心目标
- **主要目标**: 爬取拼多多社招网站的全部岗位信息  
- **成功标准**: 爬取至少200个岗位的完整信息（12个字段）  
- **预期成果**: Excel/CSV/JSON格式的岗位数据表  
- **业务价值**: 招聘市场分析、竞品调研、人才战略参考
...
## 🔗 关键链接
### API端点
- **主API**: https://careers.pddglobalhr.com/api/recruit/position/list  
- **请求方法**: POST  
- **内容类型**: application/json  
- **认证方式**: Cookie (需要 _nano_fp 等cookie)  
- **允许来源**: https://careers.pddglobalhr.com  
- **CORS策略**: 严格同源策略

### 数据源
- **数据来源**: 拼多多社招网站 (https://careers.pddglobalhr.com/jobs)  
- **数据格式**: JSON (通过API获取)  
- **更新频率**: 实时/每日更新  
- **数据量级**: 预计100-500个岗位  
- **数据字段**: 12个核心字段（参考夸克项目）
...
### 认证信息
- **认证方式**: Cookie认证  
- **主要Cookie**: _nano_fp=Xpm8lpgJn0EjXqdaXo_eBA4AmspK5Wx4Oawm5ox~  
- **凭据位置**: config/api_auth.json  
- **更新周期**: Cookie可能定期更新  
- **权限范围**: 允许访问招聘岗位列表和详情  
- **获取方式**: 通过浏览器访问网站获取
...
## 📊 数据模型
### 核心字段（从API响应中提取）
| 字段名 | API字段 | 类型 | 描述 | 示例 | 必填 | 验证规则 |
|--------|---------|------|------|------|------|----------|
| 岗位名称 | name | string | 岗位标题 | web前端高级开发工程师 | ✅ | 非空字符串 |
| 岗位代码 | code | string | 岗位唯一标识 | T015294 | ✅ | 非空字符串 |
| 工作地点 | workLocation | string | 工作城市 | 上海 | ✅ | 非空字符串 |
| 岗位类别 | job | string | 岗位分类 | 技术类 | ✅ | 非空字符串 |
| 更新时间 | updateTime | string | 更新时间字符串 | 2026-05-21 | ✅ | 日期格式 |
| 更新时间戳 | updateDate | number | 毫秒时间戳 | 1779352494000 | ✅ | 13位数字 |
| 岗位ID | code | string | 从code字段提取 | T015294 | ✅ | 非空字符串 |
| 详情链接 | 需要构造 | string | 岗位详情页URL | 需要构造 | ✅ | 有效URL格式 |
| 部门信息 | 需要解析 | string | 所属部门 | 待解析 | ⚠️ | |
| 学历要求 | 需要解析 | string | 学历要求 | 待解析 | ⚠️ | |
| 工作经验 | 需要解析 | string | 工作经验要求 | 待解析 | ⚠️ | |
| 工作职责 | 需要解析 | string | 岗位职责描述 | 待解析 | ⚠️ | |
| 任职要求 | 需要解析 | string | 任职资格要求 | 待解析 | ⚠️ | |
| 薪资范围 | 需要解析 | string | 薪资待遇 | 待解析 | ⚠️ | |
...
```json
{
  ...
  "更新时间": {
    "源字段名": "updateTime",
    "转换函数": "datetime.fromtimestamp(updateDate/1000).strftime('%Y-%m-%d')",
    "默认值": "",
    "备注": "使用updateDate时间戳转换"
  },
  "详情链接": {
    "源字段名": "code",
    "转换函数": "f'https://careers.pddglobalhr.com/jobs/{code}'",
    "默认值": "",
    "备注": "需要访问详情页获取完整信息"
  }
}
