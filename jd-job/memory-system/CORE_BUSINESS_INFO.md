...
**项目名称**: jd-job  
**创建日期**: 2026-05-22  
**最后更新**: 2026-05-22 16:30  
**项目状态**: 活跃  
**项目版本**: v1.0.0  
**项目类型**: 数据爬取  
**项目负责人**: OpenClaw Assistant

## 🎯 项目目标
### 核心目标
- **主要目标**: 爬取京东招聘网站（zhaopin.jd.com）的所有岗位信息  
- **成功标准**: 获取完整、准确的岗位数据，包含12个关键字段  
- **预期成果**: 导出Excel、CSV、JSON格式的岗位数据文件  
- **业务价值**: 为招聘分析、市场调研、竞品分析提供数据支持
...
### 数据源
- **数据来源**: 京东招聘网站（https://zhaopin.jd.com）  
- **数据格式**: JSON（通过API获取）  
- **更新频率**: 每日（岗位信息每日更新）  
- **历史数据**: 可通过分页获取历史岗位  
- **数据量级**: 预计数千个岗位
...
- **更新周期**: Cookie有效期较长，但需要定期验证  
- **权限范围**: 可访问公开的招聘岗位信息  
- **获取方式**: 从浏览器开发者工具获取有效的Cookie和请求头
