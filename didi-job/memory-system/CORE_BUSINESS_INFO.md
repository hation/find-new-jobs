...
## 🎯 项目目标
### 核心目标
- **主要目标**: 爬取滴滴出行社招岗位数据，包括岗位列表和详细信息  
- **成功标准**: 爬取所有社招岗位（jobType=3），数据完整度100%，字段准确度95%以上  
- **预期成果**: 完整的滴滴社招岗位数据库，包含岗位ID、名称、地点、部门、职责、要求等12个核心字段  
- **业务价值**: 为招聘分析、竞品研究、人才市场分析提供数据支持
...
## 🔗 关键链接
### API端点
- **列表API**: GET https://talent.didiglobal.com/recruit-portal-service/api/job/front/list  
- **详情API**: GET https://talent.didiglobal.com/recruit-portal-service/api/job/front/view/{jdId}  
- **网站地址**: https://talent.didiglobal.com/social/list/1?jobType=3  
- **生产环境**: talent.didiglobal.com  
- **监控指标**: 响应时间、成功率、数据完整性
...
### 认证信息
- **认证方式**: Cookie认证（SESSION + _OMGID + language） + Token  
- **凭据位置**: config/api_auth.json 中的 authentication 部分  
- **更新周期**: Cookie有效期未知，需要定期验证  
- **权限范围**: 可访问社招岗位列表和详情  
- **获取方式**: 从浏览器开发者工具中获取有效的Cookie和Token
...
|--------|------|------|------|------|----------|
| jdId | number | 岗位ID | 63606 | ✅ | 必须为数字，大于0 |
| jdNo | string | 岗位编号 | JR2026050600P | ✅ | 必须包含JR/J前缀 |
| jobName | string | 岗位名称 | 产品经理 | ✅ | 非空字符串 |
| workArea | string | 工作地点 | 北京市 | ✅ | 非空字符串 |
| deptName | string | 部门名称 | 平台安全部 | ✅ | 非空字符串 |
| createTime | date | 创建时间 | 2026-05-06 17:23:09 | ✅ | ISO 8601格式 |
| refreshTime | date | 刷新时间 | 2026-05-22 15:18:08 | ✅ | ISO 8601格式 |
| jobDesc | string | 工作职责 | 负责... | ✅ | 从详情页获取 |
| qualification | string | 任职要求 | 本科及以上... | ✅ | 从详情页获取 |
| jobType | number | 岗位类型 | 3 | ✅ | 3=社招 |
| recruitType | number | 招聘类型 | 1 | ✅ | 1=常规招聘 |
| recruitNum | number | 招聘人数 | 1 | ✅ | 从详情页获取 |
...
```json
{
  ...
  "createTime": {
    "源字段名": "createTime",
    "转换函数": "datetime.strptime(..., '%Y-%m-%d %H:%M:%S')",
    "默认值": null,
    "备注": "创建时间"
  },
  "refreshTime": {
...
```json
{
  "pagination": {
    ...
    "current_page_field": "data.page"
  },
  "filtering": {
    "jobType_param": "jobType",
    "recruitType_param": "recruitType",
    "jobType_value": 3,
    "recruitType_value": 1
  },
  "sorting": {
    "_comment": "未发现排序参数，按默认排序"
  }
}
```
