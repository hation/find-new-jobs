#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
夸克校园招聘API爬取器（可配置版）
符合插件规范，使用外部配置文件
"""

import json
import time
import logging
import requests
import os
from typing import Dict, List, Any, Optional
from datetime import datetime

logger = logging.getLogger(__name__)


class QuarkApiCrawlerConfigurable:
    """夸克校园招聘API爬取器（可配置版）"""
    
    def __init__(self, config_path: str = None):
        """
        初始化API爬取器（使用外部配置）
        
        Args:
            config_path: 配置文件路径，默认使用 config/api_auth.json
        """
        # 加载配置文件
        self.config_path = config_path or "config/api_auth.json"
        self.config = self._load_config()
        
        if not self.config:
            raise ValueError(f"无法加载配置文件: {self.config_path}")
        
        # 从配置中获取认证信息
        self.api_url = self.config.get("api_endpoint", "https://talent.quark.cn/position/search")
        self.csrf_token = self.config.get("authentication", {}).get("csrf_token", "")
        self.cookies = self.config.get("authentication", {}).get("cookies", {})
        self.headers = self.config.get("authentication", {}).get("headers", {})
        self.base_params = self.config.get("parameters", {})
        self.validation = self.config.get("validation", {})
        
        # 输出目录
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.output_dir = f"output/api_crawl_{timestamp}"
        os.makedirs(self.output_dir, exist_ok=True)
        
        # 状态跟踪
        self.positions_extracted = 0
        self.current_page = 1
        self.total_pages = 10
        self.max_positions = self.validation.get("expected_total_positions", 93)
        
        logger.info(f"🎯 夸克API爬取器（可配置版）初始化完成")
        logger.info(f"   配置文件: {self.config_path}")
        logger.info(f"   API端点: {self.api_url}")
        logger.info(f"   CSRF令牌: {self.csrf_token[:8]}...{self.csrf_token[-8:] if self.csrf_token else '无'}")
        logger.info(f"   Cookie数量: {len(self.cookies)} 个")
        logger.info(f"   筛选类别: {self.base_params.get('categories', '未知')}")
        logger.info(f"   输出目录: {self.output_dir}")
        logger.info(f"   目标岗位: {self.max_positions} 个")
    
    def _load_config(self) -> Optional[Dict[str, Any]]:
        """加载配置文件"""
        if not os.path.exists(self.config_path):
            logger.error(f"配置文件不存在: {self.config_path}")
            
            # 创建示例配置文件
            if self.config_path == "config/api_auth.json":
                os.makedirs("config", exist_ok=True)
                example_config = {
                    "description": "夸克API认证配置文件（示例）",
                    "last_updated": datetime.now().isoformat(),
                    "status": "example",
                    "authentication": {
                        "csrf_token": "YOUR_CSRF_TOKEN_HERE",
                        "cookies": {
                            "XSRF-TOKEN": "YOUR_XSRF_TOKEN_HERE",
                            "SESSION": "YOUR_SESSION_TOKEN_HERE"
                        },
                        "headers": {
                            "User-Agent": "Mozilla/5.0 ...",
                            "Content-Type": "application/json"
                        }
                    },
                    "api_endpoint": "https://talent.quark.cn/position/search",
                    "parameters": {
                        "categories": "97,103,143,124,152,492,146",
                        "pageIndex": 1,
                        "pageSize": 10
                    }
                }
                
                with open(self.config_path, 'w', encoding='utf-8') as f:
                    json.dump(example_config, f, ensure_ascii=False, indent=2)
                
                logger.warning(f"已创建示例配置文件: {self.config_path}")
                logger.warning("请编辑此文件，填入实际的认证信息")
            
            return None
        
        try:
            with open(self.config_path, 'r', encoding='utf-8') as f:
                config = json.load(f)
            
            logger.info(f"✅ 配置文件加载成功: {self.config_path}")
            return config
        except Exception as e:
            logger.error(f"配置文件加载失败: {e}")
            return None
    
    def validate_auth(self) -> bool:
        """验证认证信息是否有效"""
        logger.info("🔍 验证API认证信息...")
        
        if not self.csrf_token:
            logger.error("❌ CSRF令牌为空")
            return False
        
        if not self.cookies:
            logger.error("❌ Cookie为空")
            return False
        
        if "XSRF-TOKEN" not in self.cookies:
            logger.warning("⚠️  Cookie中缺少XSRF-TOKEN")
        
        if "SESSION" not in self.cookies:
            logger.warning("⚠️  Cookie中缺少SESSION")
        
        # 测试API连接
        test_result = self.fetch_page(page_index=1, page_size=1)
        
        if test_result and test_result.get("success"):
            logger.info("✅ API认证验证成功")
            return True
        else:
            logger.error("❌ API认证验证失败")
            return False
    
    def get_status(self) -> Dict[str, Any]:
        """
        获取爬取器状态
        
        Returns:
            状态字典
        """
        return {
            "type": "api_configurable",
            "config_file": self.config_path,
            "positions_extracted": self.positions_extracted,
            "current_page": self.current_page,
            "total_pages": self.total_pages,
            "max_positions": self.max_positions,
            "completion_percentage": (self.positions_extracted / self.max_positions) * 100 if self.max_positions > 0 else 0,
            "output_dir": self.output_dir,
            "csrf_token": f"{self.csrf_token[:8]}...{self.csrf_token[-8:]}" if self.csrf_token else "无",
            "has_valid_cookies": bool(self.cookies),
            "api_url": self.api_url,
            "auth_valid": self.validate_auth()
        }
    
    def fetch_page(self, page_index: int = None, page_size: int = 10) -> Optional[Dict[str, Any]]:
        """
        获取单页数据
        
        Args:
            page_index: 页码（从1开始），默认使用当前页
            page_size: 每页数量
            
        Returns:
            解析后的API响应数据，失败返回None
        """
        if page_index is None:
            page_index = self.current_page
        
        logger.info(f"📡 获取第 {page_index} 页数据 (每页 {page_size} 条)")
        
        # 构建请求参数
        params = self.base_params.copy()
        params.update({
            "pageIndex": page_index,
            "pageSize": page_size
        })
        
        # 构建完整URL（包含CSRF令牌）
        url_with_csrf = f"{self.api_url}?_csrf={self.csrf_token}"
        
        logger.debug(f"   请求URL: {url_with_csrf}")
        logger.debug(f"   请求参数: pageIndex={page_index}, pageSize={page_size}")
        
        try:
            # 发送POST请求
            response = requests.post(
                url_with_csrf,
                headers=self.headers,
                cookies=self.cookies,
                json=params,
                timeout=30,
                allow_redirects=False
            )
            
            logger.info(f"📊 响应状态码: {response.status_code}")
            
            if response.status_code == 200:
                try:
                    data = response.json()
                    
                    # 验证响应结构
                    if data.get("success") is True:
                        content = data.get("content", {})
                        positions = content.get("datas", [])
                        total_count = content.get("totalCount", 0)
                        current_page = content.get("currentPage", page_index)
                        page_size = content.get("pageSize", page_size)
                        
                        logger.info(f"✅ API请求成功!")
                        logger.info(f"📊 获取到 {len(positions)} 个岗位")
                        logger.info(f"📈 总计 {total_count} 个岗位")
                        logger.info(f"📄 当前第 {current_page} 页，每页 {page_size} 条")
                        
                        # 更新目标岗位数
                        if total_count > 0:
                            self.max_positions = total_count
                            logger.info(f"🎯 更新目标岗位数: {total_count} 个")
                        
                        return {
                            "success": True,
                            "content": content,
                            "positions": positions,
                            "total_count": total_count,
                            "current_page": current_page,
                            "page_size": page_size,
                            "raw_data": data
                        }
                    else:
                        error_msg = data.get("errorMsg", "未知错误")
                        error_code = data.get("errorCode", "未知代码")
                        logger.error(f"❌ API业务错误: {error_msg} (代码: {error_code})")
                        
                except json.JSONDecodeError as e:
                    logger.error(f"❌ JSON解析失败: {e}")
                    
            elif response.status_code == 403:
                logger.error(f"❌ 认证失败 (403 Forbidden)")
                logger.error(f"   可能原因: CSRF令牌已过期或Cookie无效")
                
            else:
                logger.error(f"❌ 请求失败: {response.status_code}")
                
        except requests.exceptions.Timeout:
            logger.error(f"⏰ 请求超时")
        except requests.exceptions.ConnectionError:
            logger.error(f"🔌 连接错误")
        except Exception as e:
            logger.error(f"❌ 请求异常: {e}")
        
        return None
    
    def crawl_page(self, page_index: int) -> List[Dict[str, Any]]:
        """
        爬取单页数据并提取岗位信息
        
        Args:
            page_index: 页码
            
        Returns:
            岗位数据列表（12个字段）
        """
        logger.info(f"📄 开始爬取第 {page_index} 页...")
        
        result = self.fetch_page(page_index)
        
        if not result or not result.get("success"):
            logger.warning(f"⚠️ 第 {page_index} 页爬取失败")
            return []
        
        positions = result.get("positions", [])
        
        if positions:
            logger.info(f"✅ 第 {page_index} 页爬取成功: {len(positions)} 个岗位")
            
            # 处理每个岗位（提取12个关键字段）
            processed_positions = []
            for i, pos in enumerate(positions):
                # 提取12个关键字段
                processed_pos = {
                    # 1. 岗位ID
                    "position_id": pos.get("id") or f"quark_{page_index}_{i+1:02d}",
                    # 2. 岗位名称
                    "position_name": pos.get("name") or "未知岗位",
                    # 3. 岗位类别
                    "position_category": ", ".join(pos.get("categories", [])) or "未知类别",
                    # 4. 工作地点
                    "work_location": ", ".join(pos.get("workLocations", [])) or "未知地点",
                    # 5. 更新时间（转换为可读格式）
                    "update_time": self._timestamp_to_date(pos.get("publishTime")),
                    # 6. 部门
                    "department": pos.get("department") or "未知部门",
                    # 7. 学历要求
                    "education_requirement": self._map_degree(pos.get("degree")),
                    # 8. 工作经验
                    "work_experience": self._format_experience(pos.get("experience")),
                    # 9. 岗位描述
                    "position_description": pos.get("description") or "",
                    # 10. 任职要求
                    "position_requirements": pos.get("requirement") or "",
                    # 11. 其他信息
                    "other_info": json.dumps({
                        "trackId": pos.get("trackId"),
                        "positionUrl": pos.get("positionUrl"),
                        "tags": pos.get("tags"),
                        "code": pos.get("code")
                    }, ensure_ascii=False),
                    # 12. 来源信息
                    "source": "api_configurable",
                    "extract_time": datetime.now().isoformat(),
                    "page_number": page_index,
                    "position_in_page": i + 1
                }
                
                processed_positions.append(processed_pos)
                self.positions_extracted += 1
            
            return processed_positions
        else:
            logger.warning(f"⚠️ 第 {page_index} 页没有获取到岗位数据")
            return []
    
    def _timestamp_to_date(self, timestamp_ms: int) -> str:
        """将毫秒时间戳转换为日期字符串"""
        if not timestamp_ms:
            return "未知时间"
        try:
            dt = datetime.fromtimestamp(timestamp_ms / 1000)
            return dt.strftime("%Y-%m-%d")
        except:
            return str(timestamp_ms)
    
    def _map_degree(self, degree: str) -> str:
        """映射学历要求"""
        degree_map = {
            "bachelor": "本科",
            "master": "硕士", 
            "doctor": "博士",
            "college": "大专",
            "high_school": "高中"
        }
        return degree_map.get(degree, degree or "学历不限")
    
    def _format_experience(self, experience: Dict) -> str:
        """格式化工作经验要求"""
        if not experience:
            return "经验不限"
        
        from_years = experience.get("from")
        to_years = experience.get("to")
        
        if from_years and to_years:
            return f"{from_years}-{to_years}年"
        elif from_years:
            return f"{from_years}年以上"
        else:
            return "经验不限"
    
    def save_positions(self, positions: List[Dict[str, Any]], page_index: int) -> str:
        """
        保存岗位数据到文件
        
        Args:
            positions: 岗位数据列表
            page_index: 页码
            
        Returns:
            保存的文件路径
        """
        if not positions:
            logger.warning(f"⚠️ 第 {page_index} 页没有数据可保存")
            return ""
        
        # 生成文件名
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"quark_page_{page_index}_{timestamp}.json"
        filepath = os.path.join(self.output_dir, filename)
        
        # 保存数据
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump({
                "metadata": {
                    "crawler_mode": "api_configurable",
                    "config_file": self.config_path,
                    "extraction_time": datetime.now().isoformat(),
                    "page_number": page_index,
                    "positions_count": len(positions),
                    "current_position": f"{self.positions_extracted}/{self.max_positions}",
                    "csrf_token": f"{self.csrf_token[:8]}...{self.csrf_token[-8:]}",
                    "categories": self.base_params.get("categories", "未知")
                },
                "positions": positions
            }, f, ensure_ascii=False, indent=2)
        
        logger.info(f"💾 已保存第 {page_index} 页 {len(positions)} 个岗位到: {filepath}")
        return filepath
    
    def crawl_all(self, start_page: int = 1, end_page: int = 10) -> Dict[str, Any]:
        """
        爬取所有页面
        
        Args:
            start_page: 起始页码
            end_page: 结束页码
            
        Returns:
            爬取结果摘要
        """
        logger.info(f"🚀 开始爬取所有页面: 第 {start_page} 页到第 {end_page} 页")
        
        # 验证认证信息
        if not self.validate_auth():
            logger.error("❌ 认证信息验证失败，无法开始爬取")
            return {
                "success": False,
                "error": "认证信息验证失败",
                "mode": "api_configurable"
            }
        
        start_time = time.time()
        all_positions = []
        successful_pages = 0
        
        for page in range(start_page, end_page + 1):
            self.current_page = page
            
            # 爬取单页
            positions = self.crawl_page(page)
            
            if positions:
                # 保存数据
                self.save_positions(positions, page)
                all_positions.extend(positions)
                successful_pages += 1
                
                # 显示进度
                completion = (self.positions_extracted / self.max_positions) * 100
                logger.info(f"📊 进度: {self.positions_extracted}/{self.max_positions} ({completion:.1f}%)")
            
            # 短暂延迟，避免请求过快
            time.sleep(1.5)
        
        # 计算耗时
        elapsed_time = time.time() - start_time
        
        # 准备结果摘要
        result = {
            "success": len(all_positions) > 0,
            "mode": "api_configurable",
            "config_file": self.config_path,
            "total_positions_extracted": len(all_positions),
            "target_positions": self.max_positions,
            "completion_percentage": (len(all_positions) / self.max_positions) * 100,
            "successful_pages": successful_pages,
            "total_pages_crawled": end_page - start_page + 1,
            "elapsed_time_seconds": elapsed_time,
            "average_time_per_page": elapsed_time / (end_page - start_page + 1) if (end_page - start_page + 1) > 0 else 0,
            "data_quality": "verified" if len(all_positions) > 0 else "failed",
            "extraction_time": datetime.now().isoformat(),
            "output_directory": self.output_dir
        }
        
        logger.info(f"🎉 爬取完成!")
        logger.info(f"📊 总计获取: {len(all_positions)} 个岗位")
        logger.info(f"🎯 目标岗位: {self.max_positions} 个")
        logger.info(f"✅ 完成比例: {result['completion_percentage']:.1f}%")
        logger.info(f"⏱️  总耗时: {elapsed_time:.2f} 秒")
        logger.info(f"📄 成功页数: {successful_pages} 页")
        
        return result


def test_configurable_api():
    """测试可配置的API爬取器"""
    print("🧪 测试可配置的API爬取器")
    print("=" * 60)
    
    # 创建爬取器
    try:
        crawler = QuarkApiCrawlerConfigurable("config/api_auth.json")
    except ValueError as e:
        print(f"❌ 爬取器创建失败: {e}")
        print("💡 请检查配置文件或运行示例配置")
        return False, 0, 0
    
    # 显示状态
    status = crawler.get_status()
    print("📊 爬取器状态:")
    for key, value in status.items():
        print(f"   {key}: {value}")
    
    print("\n📡 测试API连接...")
    
    # 测试获取第1页
    result = crawler.fetch_page(page_index=1)
    
    if result and result.get("success"):
        print("✅ API连接成功!")
        
        positions = result.get("positions", [])
        total_count = result.get("total_count", 0)
        
        print(f"🎉 总计 {total_count} 个岗位")
        print(f"📄 第1页获取 {len(positions)} 个岗位")
        
        # 显示前几个岗位
        if positions:
            print("\n📋 第1页前3个岗位:")
            for i, pos in enumerate(positions[:3]):
                name = pos.get('name', '未知标题')
                locations = ", ".join(pos.get('workLocations', []))
                categories = ", ".join(pos.get('categories', []))
                print(f"  {i+1}. {name}")
                print(f"     地点: {locations}, 类别: {categories}")
        
        return True, total_count, len(positions)
    else:
        print("❌ API连接失败")
        return False, 0, 0


def main():
    """主函数"""
    print("🚀 可配置的API爬取器测试")
    print("=" * 60)
    print("📊 使用外部配置文件，符合插件规范")
    print()
    
    success, total_count, page_count = test_configurable_api()
    
    print("\n" + "=" * 60)
    if success:
        print("🎉 可配置API方案验证成功!")
        print(f"📊 总计 {total_count} 个岗位")
        print(f"📄 第1页获取 {page_count} 个岗位")
        
        if total_count == 93:
            print("✅ 完美匹配预期 (93个岗位)")
        else:
            print(f"⚠️  预期93个岗位，但API返回 {total_count} 个")
        
        print("\n💡 下一步:")
        print("   1. 使用此爬取器完成所有页面爬取")
        print("   2. 通过配置文件管理认证信息")
        print("   3. 集成到统一入口脚本")
    else:
        print("❌ 可配置API方案验证失败")
        print("💡 可能原因:")
        print("   1. 配置文件路径错误")
        print("   2. 认证信息已过期")
        print("   3. 网络连接问题")
    
    print("=" * 60)
    
    return success


if __name__ == "__main__":
    import sys
    success = main()
    sys.exit(0 if success else 1)