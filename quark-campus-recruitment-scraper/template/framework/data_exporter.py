...
这个文件是旧版的数据导出器模板。
基于anti-job和jd-job项目的经验，我们创建了新的统一Excel导出框架。

【推荐做法】
1. 新项目：直接使用 `unified_excel_exporter.py`
2. 历史项目：逐步迁移到统一格式
3. 兼容模式：此文件保持向后兼容

【统一格式优势】
1. 标准化：16个标准列名，7个标准工作表
2. 自动化：自动数据清洗、统计分析、质量验证
3. 一致性：所有项目输出完全相同的Excel格式
4. 可维护：集中管理，一处修改，所有项目生效

【迁移步骤】
1. 导入新框架：from framework.unified_excel_exporter import UnifiedExcelExporter
2. 更新配置：使用统一列名映射配置
3. 测试验证：确保输出格式完全匹配anti-job
4. 文档更新：更新项目文档

【文件位置】
- 统一导出框架：framework/unified_excel_exporter.py
- 列名映射配置：config/excel_column_mapping.yaml
- 数据提取工具：utils/data_extractors.py
- 生成脚本：scripts/generate_unified_excel.py
- 检查清单：docs/CHECKLIST_EXCEL_EXPORT.md

【版本历史】
- v1.0.0 (2026-05-22): 基于anti-job和jd-job经验创建统一框架
- v0.9.0 (旧版): 原有的数据导出器模板
"""