...
def merge_positions(all_pages_data: List[List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    """
    合并多页数据
    ...
    for page_data in all_pages_data:
        merged_positions.extend(page_data)
    
    log_info(f"合并完成，共 {len(merged_positions)} 个岗位")
...
def save_real_data(positions: List[Dict[str, Any]]) -> Dict[str, str]:
    ...
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump({
            'metadata': {
                ...
                'pages': len(set(pos['页码'] for pos in positions)),
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"页面数量: {len(set(pos['页码'] for pos in positions))} 页\n\n")
...
def main():
    ...
    for page_num in range(start_page, end_page + 1):
        ...
        # 添加到总数据
        all_pages_data.append(page_positions)
    ...
    # 合并所有页面数据
    log_info("正在合并所有页面数据...")
    all_positions = merge_positions(all_pages_data)
