...
def main():
    ...
    backup_file = os.path.join(backup_dir, f"深圳_ai_backup_{timestamp}.xlsx")
        ...
        if job_id in df_existing['职位ID'].values:
            ...
            for col in detail_row.index:
                if col in df_existing.columns:
                    df_existing.at[row_idx, col] = detail_row[col]
            updated_count += 1
        else:
            # 添加新行
    ...
    print(f"\n✅ 合并完成！")
    print(f"💡 现在可以使用固定文件名: {fixed_excel}")
    print(f"💡 备份文件: {backup_file}")
