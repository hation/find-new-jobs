...
def get_workspace_path():
    ...
    sys.exit(1)
def main():
    print("🚀 方案B - 智能获取详情脚本 (YAML修复版)")
    print("=" * 60)
    print("💡 修复: 支持boss命令返回的YAML格式")
    ...
    for i, security_id in enumerate(unfetched[:max_attempts]):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if data and 'data' in data and data['data']:
                    ...
                    # 检查错误信息
                    if 'error' in data:
                        error_code = data['error'].get('code', '无')
        ...
        # 每个请求间隔1秒
        if i < len(unfetched[:max_attempts]) - 1 and not should_stop:
            time.sleep(1)
    ...
    print(f"\n✅ 脚本执行完成")
    print(f"💡 下次使用: python3 fetch_details_yaml_fixed.py")
